...
static void cpufreq_smartass_timer(unsigned long cpu)
{
	...
	dprintk(SMARTASS_DEBUG_LOAD,"smartassT @ %d: load %d (delta_time %d)\n",
	...
	if (cpu_load > max_cpu_load || delta_idle == 0)
	{
		...
		{
			dprintk(SMARTASS_DEBUG_ALG,"smartassT @ %d ramp up: load %d (delta_idle %d)\n",
	...
	{
		dprintk(SMARTASS_DEBUG_ALG,"smartassT @ %d ramp down: load %d (delta_idle %d)\n",
			old_freq,cpu_load,delta_idle);
		this_smartass->ramp_dir = -1;
		work_cpumask_set(cpu);
