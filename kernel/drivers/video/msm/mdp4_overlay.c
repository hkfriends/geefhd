		...
		if (s_pipe->is_fg) {
			...
			if (!alpha_drop) {
				...
				if (!(s_pipe->flags & MDP_BLEND_FG_PREMULT))
					...
					blend->fg_alpha = 0xff;
//LGE_CHANGE_S youngbok.kim@lge.com, //apply the patch of TD#264843 for GK-HK.
//DEL youngbok.kim@lge.com				blend->op |= MDP4_BLEND_BG_INV_ALPHA;
			} else
				blend->op = MDP4_BLEND_BG_ALPHA_FG_CONST;
				
			blend->op |= MDP4_BLEND_BG_INV_ALPHA; //[QCT_PATCH][SR01036939] alpha blending error
//LGE_CHANGE_E youngbok.kim@lge.com, //apply the patch of TD#264843 for GK-HK.
		} else if (d_alpha) {
			// QC_0207 : start
